...
urlpatterns = [
    ...
    path('api/orders/<int:pk>/for_calculation/', views.HistoryCheckOrderForCalculation.as_view(), name='order-for-calculation'),  # GET данные заказа для расчета (только для Go-сервиса)
    path('api/orders/result_callback/', views.HistoryCheckOrderResultCallback.as_view(), name='order-result-callback'),  # POST callback от Go-сервиса
